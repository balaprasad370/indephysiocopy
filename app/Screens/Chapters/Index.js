...
//               zIndex: 9999,
//             }}>
//             <Icon name="lock" size={30} color="#000" style={styles.lockIcon} />
//           </View>
...
const Index = ({navigation}) => {
  ...
  const chapterProgress = async chapterId => {
    ...
    if (token) {
      try {
        ...
        setProgressData(prev => ({
          ...prev,
          [chapterId]: response.data.completion_percentage,
        }));
      } catch (error) {
        console.log('Error fetching progress for chapter', chapterId, error);
      }
  ...
  const renderItem = ({item, index}) => {
    const progressPercentage = progressData[item.id] || 0;

    // Determine if this chapter is unlocked
    const isChapterUnlocked =
      index === 0 || (progressData[chapter[index - 1]?.id] || 0) === 100;
