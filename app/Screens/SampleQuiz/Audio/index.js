...
import AudioPlayer from '../components/Audio/index';
const Normal = ({item, index, isVisited, selectedAnswer, onAnswerSelect}) => {
  ...
  return (
    <ScrollView className="bg-white px-4 py-4 rounded-md" contentContainerStyle={{paddingBottom: 20}} key={`${index}-${item.id}`}>
      ...
      <View className="w-full my-2  rounded-md">
        <AudioPlayer source={{ uri: item.audioURL }} index={index} />
      </View>
