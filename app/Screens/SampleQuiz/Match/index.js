...
const JumbledSentences = ({
  ...
  const getLabelsData = async data => {
    try {
      ...
      if (!selectedAnswer) {
        ...
        const leftData = selectedAnswer.matchUserAnswerLeft;
        const leftDatabaseData = [];

        for (let i = 0; i < leftData.length; i++) {
          for (let j = 0; j < response.data.leftData.length; j++) {
            if (leftData[i] === response.data.leftData[j].value) {
              leftDatabaseData.push(response.data.leftData[j]);
            }
          }
        }

        setLeftLabelsData(leftDatabaseData);

        const rightData = selectedAnswer.matchUserAnswerRight;
        const rightDatabaseData = [];

        for (let i = 0; i < rightData.length; i++) {
          for (let j = 0; j < response.data.rightData.length; j++) {
            if (rightData[i] === response.data.rightData[j].value) {
              rightDatabaseData.push(response.data.rightData[j]);
            }
          }
        }

        setRightLabelsData(rightDatabaseData);
      }
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  const onLeftDragEnd = useCallback(
    ...
    [leftLabelsData, rightLabelsData],
  );
            ...
            renderItem={({item: columnData}) => (
              <View style={{width: (Dimensions.get('window').width - 48) / 2}}>
                <DraggableFlatList
                  ...
                  onDragEnd={
                    columnData.key === 'left' ? onLeftDragEnd : onRightDragEnd
                  }
